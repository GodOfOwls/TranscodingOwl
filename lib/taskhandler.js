...
if (os.platform() == 'win32') {
    ...
    if (!fs.existsSync(ffmpegPath)) {
        console.error('red', 'ffmpeg.exe not found, please download it from https://ffmpeg.org/download.html and place it in the bin folder');
        process.exit(1);
    }
} else {
    ...
    GlobffmpegPath = ffmpegPath;
    if (!fs.existsSync(ffmpegPath)) {
        console.error('red', 'ffmpeg not found, please download it from https://ffmpeg.org/download.html and place it in the bin folder');
        process.exit(1);
    }
}
...
class TaskHandler extends Events {
    ...
    #generateFFMPEGCommand(encoder_config, task) {
        ...
        return `"${GlobffmpegPath}" -y -i "${task}" ${metaDataTag} -c:v ${encoder_config.name} ${encoder_specific.join(" ")} -map 0 -c:a copy -c:s copy "${task.substring(0, task.lastIndexOf('.'))}-encoded.mkv"`;
    }
     ...
     */
     async #start(encoder, task) {
        ...
        if (ffmpeg_result.code !== 0) {
            ...
            // Free the encoder since the task failed
            ...
            setTimeout(() => {
                try {
                    ...
                    if ('t' in process.minimist || 'tasknotify' in process.minimist) {
                        ...
                        fetch(template(process.minimist.t || process.minimist.tasknotify, variables)).then((data) => {
                            // Consume data to prevent memory leak in node
                            data.text();
                        }).catch(e => {
                    ...
                    // This will now run after the try/catch, ensuring the encoder is freed.
                    this.encoders_free[this.encoders.indexOf(encoder)] = null; // Set encoder to free
                }
            }, 15 * 1000);
        }
    ...
    /**
     * Get the number of computed tasks (not the number of tasks in the queue)
     * @returns {Number} Number of computed tasks
     */
    getComputedTasks() {
        return this.computed_tasks;
    }
}
